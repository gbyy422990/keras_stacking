...
def build_model():
    model = Sequential()
    model.add(Conv2D(32,kernel_size=(5,5),strides=1,padding='same',activation='relu',input_shape=Input(features.shape[1:])))
...
inception_features = get_features(InceptionV3, X_test_299)
xception_features = get_features(Xception, X_test_299)

X_test_224 = get_feature_test(height_224,width_224)
resnet = get_features(ResNet50,X_test_224)
...
df2.to_csv('pred1.csv', index=None)
